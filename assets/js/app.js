...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // === loadBatch (КРИТИЧЕСКОЕ ИЗМЕНЕНИЕ: Убран фильтр по табу для глобальной загрузки) ===
  async function loadBatch(isInitial = false) { // Добавлен флаг для первой загрузки
    if (state.loading || state.end) return; 
    state.loading = true;

    const cRef = collection(db, "ursa_ipas");
    
    // 💥 КЛЮЧЕВОЕ ИЗМЕНЕНИЕ: Загружаем максимально много (200), чтобы собрать всю коллекцию для поиска
    // В Firestore нет запроса "загрузить все", поэтому используем большой лимит
    const currentLimit = isInitial ? 6 : 200; 
      
    let queryArgs = [orderBy("updatedAt", "desc"), limit(currentLimit)];
    
    // 💥 КЛЮЧЕВОЕ ИЗМЕНЕНИЕ: Фильтр по тегу применяется ТОЛЬКО при первой, ленивой загрузке.
    // Если мы уже начали скролл или поиск, мы загружаем всё, что осталось
  ...
  async function loadAllForGlobalSearch() {
    ...
    // Сброс `last` и `end`, чтобы начать загрузку всех данных с начала
    // NOTE: Если уже загружено много, этот сброс может быть не оптимален. 
    // Для простоты оставим, как в предыдущем варианте, но уберём фильтр.
    
    // 💥 КЛЮЧЕВОЕ ИЗМЕНЕНИЕ: В этом режиме мы загружаем все данные, игнорируя таб
  ...
  // === apply (КРИТИЧЕСКОЕ ИЗМЕНЕНИЕ: Фильтрация по табу применяется только при отсутствии поиска) ===
  const apply = () => {
  ...
  // === Search (КРИТИЧЕСКОЕ ИЗМЕНЕНИЕ: Принудительный глобальный режим) ===
  ...
  window.addEventListener("scroll", () => {
    // 💥 КЛЮЧЕВОЕ ИЗМЕНЕНИЕ: При активном поиске loadBatch() вызывается loadAllForGlobalSearch(),
    // поэтому scroll должен быть заблокирован при state.q.length > 0
  ...
  // === Initial load ===
  // 💥 КЛЮЧЕВОЕ ИЗМЕНЕНИЕ: При первой загрузке вызываем loadBatch(true) для загрузки только 6 карточек текущего таба
